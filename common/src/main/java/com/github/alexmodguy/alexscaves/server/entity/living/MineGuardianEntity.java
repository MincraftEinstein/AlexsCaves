    ...
    public void tick() {
        ...
        if (this.isExploding()) {
            if (explodeProgress >= 10.0F) {
                ...
                // TODO invoke event
                Explosion.BlockInteraction blockinteraction = level().getGameRules().getBoolean(GameRules.RULE_MOB_EXPLOSION_DROP_DECAY) ? Explosion.BlockInteraction.DESTROY_WITH_DECAY : Explosion.BlockInteraction.DESTROY;
