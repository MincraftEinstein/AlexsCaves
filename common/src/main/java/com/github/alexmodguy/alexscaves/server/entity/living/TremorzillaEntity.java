...
public class TremorzillaEntity extends DinosaurEntity implements KeybindUsingMount, IAnimatedEntity, ShakesScreen, KaijuMob, ActivatesSirens, ITallWalker, MultipartEntity {
    ...
    public void tick() {
        ...
        //TODO fix when fluid
        boolean water = false;//this.isInFluidType();
        ...
        //TODO fix when fluid
        if (this.onGround() && this.walkAnimation.speed() > 0.1F && !this.isBaby() && !this.isNoAi() && this.isAlive()) {
        ...
        if (level().isClientSide) {
            ...
            if (player != null && player.isPassengerOfSameVehicle(this)) {
                if (AlexsCaves.PROXY.isKeyDown(2) && getMeterAmount() >= 1.0F) {
                    NetworkRegistry.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 2));
                }
                if (AlexsCaves.PROXY.isKeyDown(3) && (this.getAnimation() == NO_ANIMATION || this.getAnimation() == null)) {
                    NetworkRegistry.sendMSGToServer(new MountedEntityKeyMessage(this.getId(), player.getId(), 3));
                }
            }
...
//        return getFluidTypeHeight(getMaxHeightFluidType());
    ...
    public void travel(Vec3 vec3d) {
        if (this.getAnimation() == ANIMATION_LEFT_STOMP || this.getAnimation() == ANIMATION_RIGHT_STOMP || this.getAnimation() == ANIMATION_LEFT_TAIL || this.getAnimation() == ANIMATION_RIGHT_TAIL || this.isFiring() && !this.isVehicle()) {
            ...
            super.travel(vec3d);
            // TODO fix when fluid
        } else if ((this.isEffectiveAi() || this.isVehicle())) {
...
//        z = event.getRatioZ();
    ...
    public boolean breakBlocksAround(Vec3 center, float radius, boolean square, boolean triggerExplosions, float dropChance) {
        // TODO invoke event
        if (this.isBaby() || level().isClientSide) {
