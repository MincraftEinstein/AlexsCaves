...
public class MagnetUtil {
    ...
    public static void tickMagnetism(Entity entity) {
        // Server side only: calculate magnetic delta from nearby magnets (POI detection requires ServerLevel)
        // MagneticEntityData is now synced to clients via AttachmentType.syncToAll()
        ...
        // Movement handling runs on both client and server (matching 1.20 behavior)
        // Client receives magneticDelta via attachment sync
            ...
            if (entity instanceof LivingEntity living) {
                ...
                if (isJumping && standingOnDirection == dir) {
                    if (entity.level().isClientSide) {
                        NetworkRegistry.sendMSGToServer(new PlayerJumpFromMagnetMessage(living.getId(), isJumping));
                    }
