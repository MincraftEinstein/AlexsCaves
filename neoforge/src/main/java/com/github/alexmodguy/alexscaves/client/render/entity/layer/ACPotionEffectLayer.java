...
public class ACPotionEffectLayer extends RenderLayer {
    ...
    private static final ResourceLocation TEXTURE_BUBBLE = ResourceLocation.fromNamespaceAndPath(AlexsCaves.MOD_ID,
    ...
    public static final ResourceLocation INSIDE_BUBBLE_TEXTURE = ResourceLocation.fromNamespaceAndPath(AlexsCaves.MOD_ID,
            "textures/misc/inside_bubble.png");
    public static final ResourceLocation TEXTURE_DARKNESS = ResourceLocation.fromNamespaceAndPath(AlexsCaves.MOD_ID,
            "textures/entity/darkness_incarnate.png");
    public static final ResourceLocation TEXTURE_SUGAR_RUSH = ResourceLocation.fromNamespaceAndPath(AlexsCaves.MOD_ID,
        ...
        if (entity instanceof LivingEntity living) {
            if (living.hasEffect(ACEffectRegistry.IRRADIATED)
                ...
                VertexConsumer ivertexbuilder = bufferIn.getBuffer(level >= IrradiatedEffect.BLUE_LEVEL
                        ...
                        : ACRenderTypes.getRadiationGlow(getTextureLocation(entity)));
            ...
            if ((living.hasEffect(ACEffectRegistry.BUBBLED) || AlexsCavesNeoForge.PROXY.hasBubbledEffectVisual(living.getId())) && living.isAlive()) {
                ...
                renderBubble(living, partialTicks, poseStack,
                        bufferIn.getBuffer(ACRenderTypes.getBubbledCull(TEXTURE_WATER)), size - 0.1F, packedLightIn,
                        size * 0.5F, size * 0.5F * 0.0625F, -0.0625F * waterAnimOffset, true);
                renderBubble(living, partialTicks, poseStack,
                        bufferIn.getBuffer(ACRenderTypes.getBubbledNoCull(TEXTURE_BUBBLE)), size, packedLightIn, 1, 1,
                        0, false);
                poseStack.popPose();
            }
    ...
    private static void renderBubble(LivingEntity entity, float partialTicks, PoseStack poseStack,
        ...
        renderCubeFace(entity, cubeAt, matrix3f, consumer, packedLight, cubeStart, cubeEnd, cubeStart, cubeEnd, cubeEnd,
                cubeEnd, cubeEnd, cubeEnd, textureScaleXZ, textureScaleY, uvOffset, water);
        renderCubeFace(entity, cubeAt, matrix3f, consumer, packedLight, cubeStart, cubeEnd, cubeEnd, cubeStart,
                cubeStart, cubeStart, cubeStart, cubeStart, textureScaleXZ, textureScaleY, uvOffset, water);
        renderCubeFace(entity, cubeAt, matrix3f, consumer, packedLight, cubeEnd, cubeEnd, cubeEnd, cubeStart, cubeStart,
                cubeEnd, cubeEnd, cubeStart, textureScaleXZ, textureScaleY, uvOffset, water);
        renderCubeFace(entity, cubeAt, matrix3f, consumer, packedLight, cubeStart, cubeStart, cubeStart, cubeEnd,
                cubeStart, cubeEnd, cubeEnd, cubeStart, textureScaleXZ, textureScaleY, uvOffset, water);
        renderCubeFace(entity, cubeAt, matrix3f, consumer, packedLight, cubeStart, cubeEnd, cubeStart, cubeStart,
                cubeStart, cubeStart, cubeEnd, cubeEnd, textureScaleXZ, textureScaleY, uvOffset, water);
        renderCubeFace(entity, cubeAt, matrix3f, consumer, packedLight, cubeStart, cubeEnd, cubeEnd, cubeEnd, cubeEnd,
                cubeEnd, cubeStart, cubeStart, textureScaleXZ, textureScaleY, uvOffset, water);
    }
    ...
    private static void renderCubeFace(LivingEntity entity, Matrix4f matrix4f, Matrix3f matrix3f,
        ...
        vertexConsumer.addVertex(matrix4f, f1, f3, f5).setColor(colorR, colorG, colorB, colorA)
                .setUv((float) 0, (float) textureScaleY + uvOffset).setOverlay(overlayCoords).setLight(packedLightIn)
                .setNormal(0.0F, -1.0F, 0.0F);
        vertexConsumer.addVertex(matrix4f, f2, f3, f6).setColor(colorR, colorG, colorB, colorA)
                .setUv((float) textureScaleXZ, (float) textureScaleY + uvOffset).setOverlay(overlayCoords)
        ...
        vertexConsumer.addVertex(matrix4f, f2, f4, f7).setColor(colorR, colorG, colorB, colorA)
                .setUv((float) textureScaleXZ, (float) uvOffset).setOverlay(overlayCoords).setLight(packedLightIn)
                .setNormal(0.0F, -1.0F, 0.0F);
        vertexConsumer.addVertex(matrix4f, f1, f4, f8).setColor(colorR, colorG, colorB, colorA)
                .setUv((float) 0, (float) uvOffset).setOverlay(overlayCoords).setLight(packedLightIn)
                .setNormal(0.0F, -1.0F, 0.0F);
    }
}
